...
const Controls: React.FC<ControlsProps> = ({ connectionState, onConnect, onDisconnect, error }) => {
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center gap-4 p-6 bg-red-900/20 rounded-lg border border-red-800">
    ...
    <div className="flex flex-col items-center gap-6">
      {connectionState === ConnectionState.DISCONNECTED ? (
        <div className="text-center">
            <button
                onClick={onConnect}
                className="group relative inline-flex items-center justify-center p-4 px-8 py-4 overflow-hidden font-medium text-indigo-400 transition duration-300 ease-out border-2 border-indigo-500 rounded-full shadow-md hover:bg-indigo-600 hover:text-white"
            >
            ...
            <p className="mt-4 text-sm text-slate-500 max-w-xs mx-auto">
        ...
        <div className="flex flex-col items-center animate-in fade-in zoom-in duration-300">
          ...
          >
            {isConnecting ? (
              <div className="w-6 h-6 border-2 border-slate-400 border-t-slate-200 rounded-full animate-spin" />
            ) : (
              <Square className="w-6 h-6 text-white fill-current" />
            )}
          </button>
          <span className="mt-3 text-xs font-medium tracking-wider text-slate-500 uppercase">
